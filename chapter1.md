...
BOOTP 自举协议（没有硬盘： 主要用于局域网）
    获得IP 地址。 是RARP的拓展：主要工作在无盘的系统上，通过发送0.0.0.0 广播，请求ip地址，然后运行该协议的 服务器就会发送服务器、客户端，网关等地址，通过tftp下载镜像到内存，进行模拟启动客户端。都是从服务器读取的。只要能跟内存通信就可以通过网络启动计算机。应用：远程维护，如果对方的计算机是坏的，此时他可以上网，就可以开发这款软件，然后重新进行远程帮他进行装系统
```
